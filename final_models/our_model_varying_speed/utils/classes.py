...
class SequentialReplayBuffer:
    ...
    def add(self, state, action, reward, done):
        """Aggiunge una singola transizione"""
    ...
    def can_sample(self, batch_size):
        return len(self.states) >= self.seq_len + batch_size

    def sample(self, batch_size):
        """Estrae batch di sequenze di lunghezza seq_len"""
        indices = []
        max_start = len(self.states) - self.seq_len
        for _ in range(batch_size):
            while True:
                idx = random.randint(0, max_start)
                # Verifica che la sequenza non attraversi un episodio finito
                if not any(itertools.islice(self.dones, idx, idx + self.seq_len - 1)):
                    indices.append(idx)
                    break
        ...
        for idx in indices:
            batch_states.append(torch.stack(list(itertools.islice(self.states, idx, idx + self.seq_len))))
            batch_actions.append(torch.stack(list(itertools.islice(self.actions, idx, idx + self.seq_len))))
            batch_rewards.append(torch.stack(list(itertools.islice(self.rewards, idx, idx + self.seq_len))))
            batch_dones.append(torch.stack(list(itertools.islice(self.dones, idx, idx + self.seq_len))))

        return (
            torch.stack(batch_states).to(self.device),     # (batch, seq_len, obs_dim)
            torch.stack(batch_actions).to(self.device),    # (batch, seq_len)
            torch.stack(batch_rewards).to(self.device),    # (batch, seq_len, reward_dim)
            torch.stack(batch_dones).to(self.device)       # (batch, seq_len)
        )
...
class QAgent():
    ...
    def act(self, current_state):
        ...
        if len(self.memory.states) < self.seq_len:
            ...
            states = list(itertools.islice(self.memory.states, -self.seq_len, None))
            actions = list(itertools.islice(self.memory.actions, -self.seq_len, None))
            rewards = list(itertools.islice(self.memory.rewards, -self.seq_len, None))

        # Stack e prepara la sequenza (1, T, dim)
        states = torch.stack(states).unsqueeze(0).to(device)     # (1, T, obs_dim)
        actions = torch.stack(actions).unsqueeze(0).to(device)   # (1, T, 1)
        rewards = torch.stack(rewards).unsqueeze(0).to(device)   # (1, T, reward_dim)
        ...
        selected_action = self.policy(q_t)
